...
class Product_Page_Calculator implements Registerable, Service {
	...
	public function register() {
        ...
        add_action( $position, [ $this, 'product_page_calculator' ] );
        add_action( 'wp_enqueue_scripts', [ $this, 'assets' ] );
        add_action( 'wp_ajax_wppsc_calculate_shipping', [ $this, 'calculate_shipping' ] );
        add_action( 'wp_ajax_nopriv_wppsc_calculate_shipping', [ $this, 'calculate_shipping' ] );
	}
    ...
    public function load_styles() {
        wp_enqueue_style( 'wppsc-select2', plugin_dir_url( $this->plugin->get_file() ) . 'assets/css/select2.css', [], $this->plugin->get_version() );

        wp_enqueue_style( 'wppsc-style', plugin_dir_url( $this->plugin->get_file() ) . 'assets/css/style.css', [], $this->plugin->get_version() );
    }

    public function load_scripts() {
        wp_enqueue_script( 'wppsc-select2', plugin_dir_url( $this->plugin->get_file() ) . 'assets/js/select2.full.js', [ 'jquery' ], $this->plugin->get_version() );

        wp_enqueue_script( 'wppsc-main', plugin_dir_url( $this->plugin->get_file() ) . 'assets/js/main.js', [ 'jquery' ], $this->plugin->get_version() );

        wp_localize_script( 'wppsc-main', 'wppsc_script', array(
